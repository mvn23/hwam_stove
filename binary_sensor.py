...
async def async_setup_entry(
    ...
    binary_sensors = []
    for var, sensor in BINARY_SENSOR_DESCRIPTIONS.items():
        device_class = sensor[0]
        name_format = sensor[1]
        entity_id = async_generate_entity_id(
            ENTITY_ID_FORMAT, f"{var}_{stove_hub.name}", hass=hass
        )
        binary_sensors.append(
            HwamStoveBinarySensor(entity_id, stove_hub, var, device_class, name_format)
        )
    for var, data in ALARM_SENSOR_DESCRIPTIONS.items():
        for sensor in data:
            device_class = sensor[0]
            name_format = sensor[1]
            alarm_name = sensor[2]
            if alarm_name is None:
                entity_id = async_generate_entity_id(
                    ENTITY_ID_FORMAT,
                    f"{var}_{stove_hub.name}",
                    hass=hass,
                )
            else:
                entity_id = async_generate_entity_id(
                    ENTITY_ID_FORMAT,
                    f"{var}_{alarm_name}_{stove_hub.name}",
                    hass=hass,
                )
            binary_sensors.append(
